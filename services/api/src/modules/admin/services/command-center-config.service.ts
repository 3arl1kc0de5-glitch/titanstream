...
export class CommandCenterConfigService {
  ...
  private seedDefaultRegistries() {
    ...
    const mm1: MobileMoneyConfig = {
      ...
      displayName: 'TetherStream UG Escrow Pool 1',
    ...
    const mm2: MobileMoneyConfig = {
      ...
      displayName: 'TetherStream UG Escrow Pool 2',
    ...
    const cw1: CryptoWalletConfig = {
      ...
      asset: 'USDT',
      network: 'TON',
      address: 'EQD_tetherstream_escrow_master_wallet_ton_001',
      label: 'TetherStream Master USDT (TON) Escrow Pool',
      status: 'ACTIVE',
      priority: 1,
  ...
  upsertMobileMoneyConfig(dto: Partial<MobileMoneyConfig>, adminId: string): MobileMoneyConfig {
    ...
    const config: MobileMoneyConfig = {
      ...
      displayName: dto.displayName || existing?.displayName || 'TetherStream Escrow Pool',
  ...
  upsertCryptoWalletConfig(dto: Partial<CryptoWalletConfig>, adminId: string): CryptoWalletConfig {
    ...
    const config: CryptoWalletConfig = {
      ...
      address: dto.address || existing?.address || 'EQD_tetherstream_escrow',
      label: dto.label || existing?.label || 'TetherStream Crypto Escrow',
