...
export const useMiningStore = create<MiningState>((set, get) => {
  ...
  return {
    ...
    tap: () => {
      ...
      if (machine.promoOutputCap && state.machineMode === 'PROMOTIONAL') {
        ...
        newLifetimePromotionalOutput = Math.min(machine.promoOutputCap, state.lifetimePromotionalOutput + tapYield);
        if (newLifetimePromotionalOutput >= machine.promoOutputCap) {
    ...
    setMultiplier: (val) => set({ coolerMultiplier: val }),
    decay: () =>
      set((state) => {
    ...
    triggerOverheat: () => set({ isOverheated: true, cooldownTimer: 15 }),
    tickCooldown: () =>
      set((state) => {
        const next = state.cooldownTimer - 1;
        if (next <= 0) {
          return {
            cooldownTimer: 0,
            isOverheated: false,
